...
public interface MyDao3 {
    ...
    @Select("""
            SELECT p.ProductName,Price,c.CategoryName
            FROM products p JOIN categories c
                ON p.CategoryID = c.CategoryID
            WHERE p.ProductID = 1
            """)
    ...
    @Select("""
             SELECT o.OrderDate `date`, ProductName name, CategoryName 
                    ,od.Quantity, Price, c.CategoryID
